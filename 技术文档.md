...
- 封装搜索工具为 LangChain Tool，保留后续扩展 Agent 的能力。
...
| `app/prompts.py` | LLM system prompt、抽取 prompt、回答 prompt |
| `app/schemas.py` | Pydantic 请求、响应、抽取结果数据结构 |
...
flowchart TD
    ...
    E --> F["选择 URL 并抓取网页正文"]
    F --> G["合并搜索摘要和网页正文"]
    G --> H["通过 LangChain ChatOpenAI 调用 LLM"]
    H --> I["LLM 返回 JSON 文本"]
    I --> J["Pydantic 校验 FootballFanAnswer"]
    J --> K["过滤博彩相关表达"]
    K --> L["终端或 API 返回结构化回答"]
```

项目没有完全交给 Agent 自由决定搜什么，而是采用“固定 query 生成器 + LLM 总结”的方式。

这样做的原因是：

- 足球问答需要稳定覆盖伤病、停赛、预计首发、战术、近况等维度。
- 直接让 Agent 自由搜索容易遗漏关键角度。
- 固定 query 更容易调试，也更容易在终端显示进度。
...
flowchart LR
    ...
    A --> D["LangChain StructuredTool"]
    D --> E["搜索工具 duckduckgo_search"]
    D --> F["网页抓取工具 fetch_url_text"]
    C --> G["JSON 文本回答"]
    G --> H["Pydantic 本地校验"]
```

当前 MVP 没有使用 LangChain 的自由 Agent 决策循环，而是使用 LangChain 作为模型调用和工具封装层。

### 5.1 使用 `ChatOpenAI` 接入模型
...
### 5.3 为什么没有继续使用 `with_structured_output`
